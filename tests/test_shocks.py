...
def test_one_sided_gaussian_shapes_are_non_negative() -> None:
    """
    ...
    shapes = generate_gaussian_factor_shapes(N, params, n_samples=500, rng=rng)

    assert np.all(shapes >= 0.0), "One-sided Gaussian shapes must be non-negative."


def test_empirical_correlation_increases_with_rho_gaussian() -> None:
    """
    ...
    Z0 = generate_gaussian_factor_shapes(N, params0, n_samples=n_samples, rng=np.random.default_rng(1))
    mean_corr0 = empirical_mean_offdiag_corr(Z0)

    params05 = ShockShapeParams(rho=0.5, one_sided=False)
    Z05 = generate_gaussian_factor_shapes(N, params05, n_samples=n_samples, rng=np.random.default_rng(2))
    mean_corr05 = empirical_mean_offdiag_corr(Z05)

    params09 = ShockShapeParams(rho=0.9, one_sided=False)
    Z09 = generate_gaussian_factor_shapes(N, params09, n_samples=n_samples, rng=np.random.default_rng(3))
...
def test_uniform_shapes_in_unit_interval() -> None:
    """
    ...
    U = generate_uniform_factor_shapes(N, params, n_samples=1000, rng=rng)

    assert np.all(U > 0.0) and np.all(U < 1.0), "Uniform shapes must be in (0,1)."


def test_empirical_correlation_increases_with_rho_uniform() -> None:
    """
    ...
    U0 = generate_uniform_factor_shapes(N, params0, n_samples=n_samples, rng=np.random.default_rng(1))
    mean_corr0 = empirical_mean_offdiag_corr(U0)

    params05 = UniformShockShapeParams(rho=0.5)
    U05 = generate_uniform_factor_shapes(N, params05, n_samples=n_samples, rng=np.random.default_rng(2))
    mean_corr05 = empirical_mean_offdiag_corr(U05)

    params09 = UniformShockShapeParams(rho=0.9)
    U09 = generate_uniform_factor_shapes(N, params09, n_samples=n_samples, rng=np.random.default_rng(3))
...
def test_generate_liquidity_drain_xi_non_negative_and_shape() -> None:
    """
    ...
    rng = np.random.default_rng(123)
    xi = generate_liquidity_drain_xi(V_ref=V, rho=0.5, lam=0.3, rng=rng, scale="row_sum")

    assert xi.shape == (3, 1)
    assert np.all(xi >= 0.0)


def test_generate_liquidity_drain_xi_is_bounded_by_lam_times_scale() -> None:
    """
    ...
    xi_row = generate_liquidity_drain_xi(V_ref=V, rho=0.2, lam=lam, rng=rng, scale="row_sum")
    s_row = V.sum(axis=1).reshape(-1, 1)
    assert np.all(xi_row <= lam * s_row + 1e-12)

    rng = np.random.default_rng(1)
    xi_col = generate_liquidity_drain_xi(V_ref=V, rho=0.2, lam=lam, rng=rng, scale="col_sum")
    s_col = V.sum(axis=0).reshape(-1, 1)
    assert np.all(xi_col <= lam * s_col + 1e-12)

    rng = np.random.default_rng(1)
    xi_tot = generate_liquidity_drain_xi(V_ref=V, rho=0.2, lam=lam, rng=rng, scale="total")
...
def test_generate_liquidity_drain_xi_correlation_increases_with_rho() -> None:
    """
    Empirically, dependence in xi across nodes should increase with rho,
    holding everything else fixed.

    We generate many draws by varying the RNG seed deterministically.
    """
    N = 25
    ...
    X_rho0 = np.vstack([
        generate_liquidity_drain_xi(V_ref=V, rho=0.0, lam=lam, rng=np.random.default_rng(s), scale="row_sum").T
        for s in range(n_samples)
    ])

    X_rho9 = np.vstack([
        generate_liquidity_drain_xi(V_ref=V, rho=0.9, lam=lam, rng=np.random.default_rng(s), scale="row_sum").T
        for s in range(n_samples)
    ])
